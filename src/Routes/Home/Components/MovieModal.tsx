...
export default function MovieInfoModal({ movie }: { movie: MovieResponse })
{
    ...
    return createPortal((
        <div className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[80%] h-fit bg-[#252525] z-50 overflow-auto rounded-2xl flex justify-between'>
            <div className='flex flex-col z-1 p-10 gap-4 w-150'>
                <h1 className='text-6xl text-white'>{movie.title}</h1>

                <div className='grid grid-flow-col gap-4'>
                    <h1 className='text-xl font-bold text-green-400'>{`Avaliações: ${Math.round(movie.rating/10 * 100)}%`}</h1>
                    <h1 className='text-xl text-white'>{`Lançamento: ${new Date(movie.releaseDate).toLocaleDateString('en-ca')}`}</h1>
                </div>

                <p className='text-xl text-white/80 max-w-100'>{movie.synopsis}</p>
            ...
            <div>
                <img className='object-cover h-full' src={movie.backdropPath} />
